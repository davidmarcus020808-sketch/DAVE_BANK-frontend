    ...
    <motion.div className="min-h-screen flex flex-col bg-black text-yellow-400 font-[Inter]" initial="hidden" animate="visible" transition={{ staggerChildren: 0.2 }}>
      <div className="flex-1 px-8 py-10 space-y-10">
        ...
        <motion.div variants={fadeInUp}>
          <h1 className="text-3xl font-bold text-yellow-300 mb-2 tracking-tight">
            Dashboard Overview
          </h1>
        ...
        <motion.div className="flex gap-4 mb-4" variants={fadeInUp}>
          <select className="bg-black text-yellow-400 border border-yellow-700 rounded px-3 py-2" value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}>
            <option value="All">All</option>
            {Object.keys(categoryMap).map((cat) => <option key={cat} value={cat}>{cat}</option>)}
          </select>
          <select className="bg-black text-yellow-400 border border-yellow-700 rounded px-3 py-2" value={selectedDuration} onChange={e => setSelectedDuration(e.target.value)}>
        ...
        {/* Summary Cards */}
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" variants={{ visible: { transition: { staggerChildren: 0.15 } } }}>
          {summaryItems.map((item, i) => (
            <motion.div key={i} className={`p-5 rounded-2xl bg-gradient-to-br ${item.color} shadow-[0_0_15px_rgba(255,215,0,0.15)] border border-yellow-700/30 flex items-center justify-between`} variants={fadeInUp} whileHover={{ scale: 1.03 }}>
              <div>
                <h2 className="text-sm text-yellow-200/70">{item.title}</h2>
                <p className="text-xl font-bold text-yellow-300 mt-1">₦{Number(item.value).toLocaleString()}</p>
              </div>
        ...
        {/* Charts */}
        <motion.div className="grid grid-cols-1 lg:grid-cols-2 gap-6" variants={{ visible: { transition: { staggerChildren: 0.25 } } }}>
          {/* Bar Chart */}
          <motion.div className="bg-gradient-to-br from-[#1a1a1a] to-[#2a1a00] border border-yellow-700/20 rounded-2xl p-6 shadow-[0_0_15px_rgba(255,215,0,0.15)]" variants={fadeInUp}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-yellow-300">Income vs Expenses</h3>
          ...
          <motion.div className="bg-gradient-to-br from-[#1a1a1a] to-[#2a1a00] border border-yellow-700/20 rounded-2xl p-6 shadow-[0_0_15px_rgba(255,215,0,0.15)]" variants={fadeInUp}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-yellow-300">Spending Breakdown</h3>
            ...
            <div className="w-full h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: "#1a1a1a", border: "1px solid #fcd34d", borderRadius: "8px", color: "#fcd34d" }} />
                  <Legend verticalAlign="bottom" align="center" wrapperStyle={{ color: "#fcd34d", fontSize: "12px" }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
        ...
        {/* Security Section */}
        <motion.div className="bg-gradient-to-br from-[#1a1a1a] to-[#2a1a00] border border-yellow-700/20 rounded-2xl p-6 shadow-[0_0_15px_rgba(255,215,0,0.15)]" variants={fadeInUp}>
          <h3 className="text-lg font-semibold text-yellow-300 mb-4">Security & Performance</h3>
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { title: "System Status", desc: "All systems operational", icon: <ShieldCheck size={24} className="text-yellow-400" />, status: "Active" },
              { title: "Card Protection", desc: "Advanced fraud monitoring", icon: <CreditCard size={24} className="text-yellow-400" />, status: "Enabled" },
              { title: "Performance", desc: "Last updated: 2 mins ago", icon: <BarChart3 size={24} className="text-yellow-400" />, status: "Optimized" },
            ].map((sec, i) => (
              <motion.div key={i} className="bg-yellow-900/10 p-4 rounded-xl border border-yellow-700/20 flex justify-between items-center" variants={fadeInUp} whileHover={{ scale: 1.03 }}>
                <div className="flex items-center gap-3">
                  {sec.icon}
                  <div>
                    <p className="font-medium text-yellow-300">{sec.title}</p>
                    <p className="text-yellow-200/70 text-xs">{sec.desc}</p>
                  </div>
                </div>
                <span className="text-green-400 font-semibold text-sm">{sec.status}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        {/* CTA */}
        <motion.div className="text-center py-10" variants={fadeInUp} transition={{ delay: 0.2 }}>
          <p className="text-yellow-200/80 text-sm mb-3">Experience next-level banking with Dave Bank.</p>
          <motion.button className="bg-yellow-400 text-black px-6 py-3 rounded-lg font-semibold hover:bg-yellow-300 hover:shadow-[0_0_15px_rgba(255,215,0,0.8)] transition-all duration-300" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
